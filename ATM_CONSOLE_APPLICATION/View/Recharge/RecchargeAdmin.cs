        ...
        public override void Table_Recharge()
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        table.AddRow($"{item.ID_Transaction}", $"{item.Bank_Account.Number_Bank}", $"{item.User.FullName}", $"{item.User.CMND_CCCD}", $"{item.Bank_Account.Number_Bank}", $"{item.Bank_Account.Balance}", $"{item.User.Email}", $"{item.User.Phone}", $"{item.amount}", $"{item.status_transaction}", $"{item.created_at_transaction}");
                    }
                }
